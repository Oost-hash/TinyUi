...
def main() -> int:
    ...
    dialog_qml = paths.source_root / "qml_app" / "SettingsDialogWindow.qml"
    ...
    opened = _pump_events(args.open_timeout, lambda: root.isVisible() and settings_vm.open)
    if not opened:
    ...
    closed = _pump_events(args.close_timeout, lambda: (not root.isVisible()) and (not settings_vm.open))
    if not closed:
