...
@dataclass
class SuspensionForce(WidgetConfig):
    name: str = "suspension_force"

    # groups
    bar: BarConfig = field(default_factory=BarConfig)
    font: FontConfig = field(default_factory=FontConfig)
    position: PositionConfig = field(default_factory=lambda: PositionConfig(position_x=391, position_y=713))

    # cells
    force_ratio: CellConfig = field(default_factory=lambda: CellConfig(id='force_ratio'))

    # components
    caption: CaptionConfig = field(default_factory=lambda: CaptionConfig(caption_text='susp force', show=False))

    # config
    font_color: str = '#AAAAAA'
    highlight_color: str = '#CC00CC'
    ...
    def to_flat(self) -> Dict[str, Any]:
        ...
        result.update(self.force_ratio.to_flat())
    ...
    @classmethod
    def from_flat(cls, data: Dict[str, Any]) -> "SuspensionForce":
        obj = cls()
        ...
        obj.force_ratio = CellConfig.from_flat(data, 'force_ratio')
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result["force_ratio"] = self.force_ratio.to_dict()
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SuspensionForce":
        obj = cls()
        for key in ("enable", "update_interval", "opacity",
                    "bar_gap", "bar_padding", "layout", "bkg_color"):
            if key in data:
        ...
        obj.force_ratio = CellConfig.from_dict(data.get("force_ratio", {}), 'force_ratio')
