...
@dataclass
class TrackNotes(WidgetConfig):
    name: str = "track_notes"

    # groups
    font: FontConfig = field(default_factory=FontConfig)
    position: PositionConfig = field(default_factory=lambda: PositionConfig(position_x=739, position_y=182))
    ...
    track_notes: CellConfig = field(default_factory=lambda: CellConfig(id='track_notes', font_color=NOTES.font_color, bkg_color=NOTES.bkg_color, column_index=1))
    ...
    track_notes_text_alignment: str = 'Center'
    track_notes_uppercase: bool = False
    track_notes_width: int = 30
    ...
    def to_flat(self) -> Dict[str, Any]:
        ...
        result.update(self.track_notes.to_flat())
        ...
        result["track_notes_text_alignment"] = self.track_notes_text_alignment
        result["track_notes_uppercase"] = self.track_notes_uppercase
        result["track_notes_width"] = self.track_notes_width
        return result

    @classmethod
    def from_flat(cls, data: Dict[str, Any]) -> "TrackNotes":
        obj = cls()
        ...
        obj.track_notes = CellConfig.from_flat(data, 'track_notes')
        ...
        obj.track_notes_text_alignment = data.get("track_notes_text_alignment", obj.track_notes_text_alignment)
        obj.track_notes_uppercase = data.get("track_notes_uppercase", obj.track_notes_uppercase)
        obj.track_notes_width = data.get("track_notes_width", obj.track_notes_width)
        return obj
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        result["track_notes"] = self.track_notes.to_dict()
        result["auto_hide_if_not_available"] = self.auto_hide_if_not_available
        result["comments_text_alignment"] = self.comments_text_alignment
        result["comments_width"] = self.comments_width
        result["debugging_text_alignment"] = self.debugging_text_alignment
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "TrackNotes":
        obj = cls()
        for key in ("enable", "update_interval", "opacity",
                    "bar_gap", "bar_padding", "layout", "bkg_color"):
            if key in data:
        ...
        obj.track_notes = CellConfig.from_dict(data.get("track_notes", {}), 'track_notes')
        obj.auto_hide_if_not_available = data.get("auto_hide_if_not_available", obj.auto_hide_if_not_available)
        obj.comments_text_alignment = data.get("comments_text_alignment", obj.comments_text_alignment)
        obj.comments_width = data.get("comments_width", obj.comments_width)
        obj.debugging_text_alignment = data.get("debugging_text_alignment", obj.debugging_text_alignment)
