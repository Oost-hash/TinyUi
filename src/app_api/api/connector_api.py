"""Connector API exposed to QML for runtime connector-backed providers."""
...
class ConnectorApi(QObject):
    """Expose connector-backed provider snapshots and controls to QML."""
    ...
    def inspectionRows(self, provider_id: str) -> list[dict]:
        return [
            {"key": key, "value": value}
            for key, value in self._providers.inspect(provider_id)
        ]
