...
# tinyui/ui/components/title_bar.py
...
def _images_dir() -> str:
    """Bepaal het pad naar de map met afbeeldingen (werkt ook in een gebundelde executable)."""
...
class TitleBar(QFrame):
    ...
    def update_maximize_icon(self, maximized: bool):
        """
        Pas het icoon van de maximaliseerknop aan op basis van de vensterstatus.
        Wordt aangeroepen vanuit FramelessWindow wanneer de status verandert.
        """
    ...
    def _toggle_maximize(self):
        """Wissel tussen normaal en gemaximaliseerd."""
        win = self.window()
        if win.isMaximized():
            win.showNormal()
        else:
            win.showMaximized()
        # Icoon wordt automatisch bijgewerkt via changeEvent in FramelessWindow

    # ----- Native slepen van het venster -----
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            win = self.window()
            # Als het venster gemaximaliseerd is, eerst herstellen (standaard gedrag)
            if win.isMaximized():
                win.showNormal()
            # Start native verplaatsing
            if win.windowHandle():
