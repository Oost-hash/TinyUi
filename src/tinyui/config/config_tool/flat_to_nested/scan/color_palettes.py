...
def extract_color_keys(config: Dict[str, Any]) -> List[Tuple[str, str]]:
    ...
    Returns lijst van (key, hex_value) tuples.
    """
...
def extract_color_pairs(config: Dict[str, Any]) -> List[Dict]:
    ...
    Returns lijst van palette dicts met suffix, font_color, bkg_color.
    """
    font_colors = {}
    bkg_colors = {}
...
def match_unpaired_to_palette(
    ...
    Als een bkg_color of font_color alleen voorkomt maar die hex waarde
    zit in een bekend palette, koppel hem daaraan.
    """
    # Index palettes op font en bkg
    by_font = {}
    by_bkg = {}
    ...
    for entry in unpaired:
        ...
        if bkg and not font and bkg in by_bkg:
            # Heeft alleen bkg, zoek palette waar deze bkg in zit
            candidates = by_bkg[bkg]
            # Pak het meest voorkomende palette (eerste in lijst = meest common)
            match = {"font_color": candidates[0][0], "bkg_color": candidates[0][1]}
        elif font and not bkg and font in by_font:
            # Heeft alleen font, zoek palette waar deze font in zit
...
def analyze_widget_colors(widget_name: str, config: Dict[str, Any]) -> Dict:
    ...
    # Splits paren: compleet (font+bkg) vs ongepaarde
...
def analyze_all_colors(configs: Dict[str, Dict]) -> Dict:
    ...
    Matcht ongepaarde kleuren aan bekende palettes zodat
    uiteindelijk alleen unieke palettes overblijven.
    """
    # Stap 1: verzamel alle complete paren over alle widgets
    ...
    # Sorteer palettes op frequentie voor matching
    sorted_palettes = [p for p, _ in palette_counter.most_common()]

    # Stap 2: match ongepaarde kleuren aan bekende palettes
    per_widget = {}
    total_matched = 0
