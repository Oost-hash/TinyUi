...
"""Image source abstraction for filesystem vs QRC paths."""
...
class ImageSource:
    """Reference to an image asset that works in both dev and build mode.
    ...
    @classmethod
    def filesystem(cls, path: Path) -> ImageSource:
        """Create an image source that loads from the filesystem."""
        return cls(_filesystem_path=path)

    @classmethod
    def qrc(cls, path: str) -> ImageSource:
        """Create an image source that loads from Qt resources (qrc:/)."""
        if not path.startswith("/"):
            path = "/" + path
        return cls(_qrc_path=path)

    @classmethod
    def dual(cls, filesystem_path: Path, qrc_path: str) -> ImageSource:
        """Create an image source that works in both modes.
    ...
    def to_url(self, frozen: bool | None = None) -> QUrl:
        """Return a QUrl suitable for QML Image.source.
