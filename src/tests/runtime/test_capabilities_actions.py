...
def test_connector_actions_delegate_to_registry_and_emit_on_success() -> None:
    ...
    registry.register("telemetry_connector", "telemetry_connector", "Telemetry", provider)
    capability = ConnectorActions(registry)
    changed: list[str] = []
    capability.providerDataChanged.connect(changed.append)
    ...
    assert capability.updateProvider("telemetry_connector") is True

    assert provider.requests == [("devtools", "mock")]
    assert provider.releases == ["devtools"]
    assert provider.updated == 1
    assert changed == ["telemetry_connector", "telemetry_connector", "telemetry_connector"]


def test_connector_actions_return_false_when_provider_is_missing() -> None:
    """ConnectorActions should fail cleanly for unknown providers."""
    capability = ConnectorActions(ProviderRegistry())

    assert capability.requestSource("missing", "devtools", "mock") is False
    assert capability.releaseSource("missing", "devtools") is False
    assert capability.updateProvider("missing") is False
